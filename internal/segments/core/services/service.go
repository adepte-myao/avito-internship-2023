...
type Service struct {
	...
	userServiceProvider ports.UserServiceProvider
	userLocalProvider   ports.UserProvider
	segmentsProvider    ports.SegmentsProvider
	historyProvider     ports.UserSegmentHistoryProvider
	deadlineAdder       ports.DeadlineWorker
	fileStorage         ports.FileStorage
}

func NewService(logger common.Logger, providerContext context.Context, userServiceProvider ports.UserServiceProvider,
	userLocalProvider ports.UserProvider, segmentsProvider ports.SegmentsProvider, historyProvider ports.UserSegmentHistoryProvider,
	deadlineAdder ports.DeadlineWorker, fileStorage ports.FileStorage) *Service {
	return &Service{
		logger:              logger,
...
func (service *Service) ChangeSegmentsForUser(dto ports.ChangeSegmentsForUserDTO) error {
...
func (service *Service) validateSegmentEntries(ctx context.Context, toAdd []ports.AddSegmentEntry, toRemove []ports.RemoveSegmentEntry) error {
...
func (service *Service) addDeadlines(userID string, segmentsToAdd []ports.AddSegmentEntry) error {
	deadlines := make([]domain.DeadlineEntry, 0)
	for _, segmentInfo := range segmentsToAdd {
		...
		deadlines = append(deadlines, domain.DeadlineEntry{
			UserID:   userID,
...
func (service *Service) GetSegmentsForUser(dto ports.GetSegmentsForUserDTO) (ports.GetSegmentsForUserOutDTO, error) {
	execCtx, cancelExec := context.WithTimeout(service.providerContext, time.Second)
	defer cancelExec()

	slugs, err := service.segmentsProvider.GetForUser(execCtx, dto.UserID)
	if err != nil {
		return ports.GetSegmentsForUserOutDTO{}, err
	}

	return ports.GetSegmentsForUserOutDTO{Segments: slugs}, nil
}

func (service *Service) GetHistoryReportLink(dto ports.GetSegmentsHistoryReportLinkDTO) (string, error) {
	execCtx, cancelExec := context.WithTimeout(service.providerContext, time.Second)
	defer cancelExec()
...
func (service *Service) CreateSegment(dto ports.CreateSegmentDTO) error {
	execCtx, cancelExec := context.WithTimeout(service.providerContext, time.Second)
	defer cancelExec()
...
func (service *Service) createSegment(ctx context.Context, dto ports.CreateSegmentDTO) error {
	err := service.validateSegmentsNotExist(ctx, []string{dto.Slug})
	if err != nil {
		return err
	}

	err = service.segmentsProvider.Create(ctx, domain.Segment{Slug: dto.Slug})
	if err != nil {
		service.logger.Error(err)
		return err
	}
...
func (service *Service) fillSegment(ctx context.Context, segmentInfo ports.CreateSegmentDTO) error {
...
func (service *Service) RemoveSegment(dto ports.RemoveSegmentDTO) error {
	execCtx, cancelExec := context.WithTimeout(service.providerContext, time.Second)
	defer cancelExec()
...
func (service *Service) CreateUser(dto ports.CreateUserDTO) error {
	execCtx, cancelExec := context.WithTimeout(service.providerContext, time.Second)
	defer cancelExec()

	return service.userLocalProvider.Create(execCtx, domain.User{Id: dto.UserID, Status: domain.Active})
}

func (service *Service) UpdateUser(dto ports.UpdateUserDTO) error {
	if err := domain.ValidateUserStatus(dto.Status); err != nil {
		return err
	}
	...
	return service.userLocalProvider.Update(execCtx, domain.User{Id: dto.UserID, Status: dto.Status})
}

func (service *Service) RemoveUser(dto ports.RemoveUserDTO) error {
	execCtx, cancelExec := context.WithTimeout(service.providerContext, time.Second)
	defer cancelExec()
...
func (service *Service) ProcessUserAction(userID string) {
	...
	targetUser := domain.User{Id: userID, Status: userStatus}
	if exists {
