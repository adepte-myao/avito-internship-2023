...
func (repo *SegmentRepository) GetAllAsMap(ctx context.Context) (map[string]domain.Segment, error) {
	...
	segmentsMap := make(map[string]domain.Segment)
	for rows.Next() {
...
func (repo *SegmentRepository) Create(ctx context.Context, segment domain.Segment) error {
	executor, err := postgres.GetExecutor(ctx, repo.db)
	if err != nil {
		return err
	}
...
func (repo *SegmentRepository) prepareHistoryEntriesForSegmentRemove(ctx context.Context, slug string) ([]domain.UserSegmentHistoryEntry, error) {
	executor, err := postgres.GetExecutor(ctx, repo.db)
	if err != nil {
		return nil, err
	}
	...
	historyEntries := make([]domain.UserSegmentHistoryEntry, len(userIDsInSegment))
...
func (repo *SegmentRepository) AddUsersToSegments(ctx context.Context, userIDs, slugs []string) error {
	...
	historyEntries := make([]domain.UserSegmentHistoryEntry, len(userIDs)*len(slugs))
	for i, userID := range userIDs {
		for j, slug := range slugs {
			index := i*len(slugs) + j
			historyEntries[index] = domain.UserSegmentHistoryEntry{
				UserID:     userID,
				Slug:       slug,
				ActionType: domain.Added,
				LogTime:    time.Now(),
			}
		}
	}
...
func (repo *SegmentRepository) RemoveSegmentsForUser(ctx context.Context, userID string, slugsToRemove []string) error {
	...
	historyEntries := make([]domain.UserSegmentHistoryEntry, len(slugsToRemove))
