...
func (repo *UserSegmentDeadlineRepository) AddDeadlines(ctx context.Context, deadlines []domain.DeadlineEntry) error {
	if len(deadlines) == 0 {
		return nil
	}
...
func (repo *UserSegmentDeadlineRepository) GetAllBefore(ctx context.Context, maxTime time.Time) ([]domain.DeadlineEntry, error) {
	executor, err := postgres.GetExecutor(ctx, repo.db)
	if err != nil {
		return nil, err
	}
	...
	deadlineEntries := make([]domain.DeadlineEntry, 0)
	for rows.Next() {
		var entry domain.DeadlineEntry
...
func (repo *UserSegmentDeadlineRepository) Remove(ctx context.Context, toRemove []domain.DeadlineEntry) error {
	executor, err := postgres.GetExecutor(ctx, repo.db)
	if err != nil {
		return err
	}
